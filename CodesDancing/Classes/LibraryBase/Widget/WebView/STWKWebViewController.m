...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if ([message.body hasPrefix:kURLPrefix]) {
        ...
        [STBridgeDefaultCommunication handleBridge:self functionName:functionName params:jsonStr callBackId:callbackId callback:^(NSString * _Nullable callBackId, NSString * _Nullable resultJsonString ){
            NSString *jsStr = [NSString stringWithFormat:@"window.bridge.onCallback(%@,'%@')", callbackId, resultJsonString];
